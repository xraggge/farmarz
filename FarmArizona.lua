...
if not doesDirectoryExist('moonloader/config/FarmArizona') then
	createDirectory('moonloader/config/FarmArizona')
end

if not doesDirectoryExist('moonloader/resource/FarmArizona/fonts') then
	createDirectory('moonloader/resource/FarmArizona/fonts')
end

local path = getWorkingDirectory() .. "\\config\\FarmArizona"
...
if not doesFileExist('moonloader/config/FarmArizona/Settings.ini') then
	inicfg.save(mainIni,'FarmArizona/Settings.ini')
end
...
changelog5 = [[
	...
	v3.0 Beta
		...
		· Добавлена светлая FarmArizona тема [Beta]
		...
		· Config преобразован в Settings.ini
		· Рабочая директория конфига - /moonloader/config/FarmArizona/...
...
scriptinfo = [[
 FarmArizona - скрипт, для прокачки аккаунта на Arizona Role Play!
В данном разделе вы можете найти ссылки на соц-сети проекта(FarmArizona), тем самым больше узнать о скрипте.
]]
...
prefixtext = [[
Префиксы оформления:
[FarmArizona] - темы сделанные на основе легендарной, стандартной, родной темы FarmArizona.
...
local banner = imgui.CreateTextureFromFile(getWorkingDirectory() .. "\\resource\\FarmArizona\\script_banner.png")
...
function libs()
	downloadUrlToFile('https://github.com/xraggge/farmarz/raw/main/font/fontawesome-webfont.ttf',
	'moonloader\\resource\\FarmArizona\\Fonts\\fontawesome-webfont.ttf', 
	'fontawesome-webfont.ttf')
	
	downloadUrlToFile('https://github.com/SMamashin/AFKTools/raw/main/resource/AFKTools/script_banner.png',
	'moonloader\\resource\\FarmArizona\\script_banner.png', 
	'script_banner.png')
	...
	for k, v in ipairs(_message) do
		...
		if count < 10 then
			...
			if v.active then
				...
				imgui.RenderMsgLogo() imgui.SameLine() imgui.CenterText("FarmArizona")
				imgui.Separator()
...
function imgui.BeforeDrawFrame()
    if fa_font == nil then
        ...
        fa_font = imgui.GetIO().Fonts:AddFontFromFileTTF('moonloader/resource/FarmArizona/fonts/fontawesome-webfont.ttf', 15, font_config, fa_glyph_ranges)
...
function imgui.OnDrawFrame()
	if afksets.v then
		...
		imgui.SetCursorPosX(375) -- позволяет задать положение функции по горизнотали
		imgui.SetCursorPosY(80) -- позволяет задать положение функции по вертикали
 		imgui.TextColoredRGB("Аккаунт: {FF0000} " ..acc) 
		imgui.SetCursorPos(imgui.ImVec2(40,8)) -- Author: neverlane(ronnyevans)\n
		imgui.RenderLogo() imgui.SameLine() imgui.Text(u8('\nscript developer: xraggge'))
		...
		elseif menunum == 1 then
			...
			PaddingSpace()
			imgui.BeginGroup()
			if imgui.Checkbox(u8('Быстрое подключение к серверу'),fastconnect) then
				sampFastConnect(fastconnect.v)
			end
			imgui.SameLine()
	...
	imgui.TextQuestion(u8"Функция удаляет всех игроков в радиусе. Очень полезно при скупе т.к падает шанс краша игры. Чтобы вернуть игроков - выключите функцию и зайдите в инту, затем выйдите из неё. Или можно просто перезайти в игру.")
			imgui.Checkbox(u8('Автологин(лаунчер)'),autologinfix.state)
			imgui.SameLine()
			imgui.TextQuestion(u8('Позволяет автоматически авторизовывать аккаунт с лаунчера.'))
			...
			PaddingSpace()
			imgui.Separator()
			imgui.CenterText(u8('Необходимые библиотеки'))
			imgui.Separator()
			PaddingSpace()
			imgui.BeginGroup()
			-- Libs --
		...
		elseif menunum == 3 then
			...
			imgui.Separator()
			imgui.BeginGroup()
        	imgui.RadioButton(u8'Отключить',eat.eatmetod,0)
		...
		elseif menunum == 4 then
			...
			if imgui.Button(fa.ICON_VK .. u8(' - ВКонтакте')) then
				os.execute("start https://vk.com/xraggge")
			end
			imgui.SameLine()
			if imgui.Button(fa.ICON_TELEGRAM .. u8(' - Telegrm')) then
...
function onScriptTerminate(scr,qgame)
	if scr == thisScript() then
		local saved = inicfg.save(mainIni,'FarmArizona/Settings.ini')
...
updates.data = {
	...
	relevant_version = '',
	url_update = '',
	url_json = 'https://raw.githubusercontent.com/xraggge/farmarz/main/updater.json' 
}
